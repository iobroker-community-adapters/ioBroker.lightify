...
This adapter connects ioBroker to OSRAM Lightify smart lighting systems. It communicates with the OSRAM Lightify Gateway via network protocol to control smart bulbs, light strips, and other OSRAM/Sylvania Lightify-compatible devices.

**Key Features:**
- Control OSRAM Lightify bulbs (on/off, brightness, color, temperature)
- Support for groups and zones configured in Lightify app
- Auto-discovery of Lightify Gateway via mDNS
- Polling-based status updates from gateway
- JSON command interface similar to Hue adapter

**Hardware Requirements:**
- OSRAM Lightify Gateway connected to local network
- OSRAM Lightify compatible smart bulbs/devices configured via Lightify mobile app

**Network Protocol:**
- Uses proprietary OSRAM protocol to communicate with gateway
- mDNS discovery for automatic gateway detection
...
## Lightify Adapter Specific Guidelines

### Command State Pattern
The adapter uses a JSON command interface similar to Hue adapter:
- Support identifiers: `red, r, green, g, blue, b, bri, sat, transition, on, off`
- Accept both JSON and assignment syntax: `{r:100, b: 200, transition: 20}` or `r = 100; g = 250, b = 100`
- Color range: 0-255, brightness range: 0-100
- Maintain backwards compatibility with existing command formats

### Network Communication
- Use mDNS discovery for gateway auto-detection (mdns-discovery package)
- Implement proper connection state management and retry logic
- Handle network timeouts gracefully with appropriate logging
- Support manual IP configuration as fallback

### Device State Management
- Map device states to match Hue adapter patterns for consistency
- Implement proper state caching to reduce gateway polling
- Handle device availability changes (bulbs turned off at switch)
- Support both individual devices and Lightify groups/zones

### Error Handling Patterns
- Provide clear error messages for common issues (gateway not found, device unreachable)
- Implement exponential backoff for connection retries
- Log network communication issues at appropriate levels (debug for polling, warn for connection issues)

### Configuration Management
- Support both auto-discovery and manual IP configuration
- Validate polling intervals (minimum 5 seconds to avoid gateway overload)
- Preserve device configurations during adapter restarts